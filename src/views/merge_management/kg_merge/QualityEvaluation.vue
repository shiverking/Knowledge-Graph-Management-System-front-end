  ...
  <div style="margin-top: 20px;">
    ...
    <el-tabs v-model="activeName" type="card" @tab-click="handleClick" style="margin-top:10px">
      <el-steps :active="active" finish-status="success" simple>
        <el-step title="链接异常（本体规则检测）"></el-step>
        <el-step title="链接异常（智能匹配检测）"></el-step>
      ...
      <div v-if="this.active==0">
        ...
        <el-dialog title="异常修改" :visible.sync="dialogRelationErrorFormVisible" customClass="customWidth" fullscreen>
          ...
          <el-card shadow="never" style="">
            <el-table :data="single_relation_error" border style="width: 100%">
      ...
      <div v-if="this.active==1">
        <el-button type="primary" @click="get_abnormal_by_algorithm()">开始检测</el-button>
        <el-popover placement="top" v-model="isJoin">
          <p>是否应用链接异常的修改结果？</p>
          <div style="text-align: right; margin: 0">
            <el-button size="mini" type="text" @click="isJoin = false">取消</el-button>
            <el-button type="primary" size="mini" @click="isJoin = false; join_kgistSubmission_form(); link_modified_successfully();">确定</el-button>
          </div>
          <el-button type="success" slot="reference" style="margin-top: 10px; margin-left: 10px;">应用</el-button>
        </el-popover>
        <keep-alive>
          <el-table :v-loading="loading" :data="tableData1" border :row-class-name="tableRowClassName" style="width: 100%; margin-top:10px">
            ...
            <el-table-column label="尾实体" width="320">
              <template slot-scope="scope">
                <span style="margin-left: 10px">{{ scope.row.tail }}</span>
                <el-tag size="small" type="info">{{ scope.row.tail_typ }}</el-tag>
              </template>
            ...
            <el-table-column prop="abnormal_score" label="异常分数"></el-table-column>
            <el-table-column label="错误状态">
              <template slot-scope="scope">
                <span v-if="scope.row.error_status == 0" style="margin-left: 10px">待处理</span>
                <span v-if="scope.row.error_status == 1" style="margin-left: 10px">待提交</span>
            ...
            <el-table-column label="操作">
              <template slot-scope="scope">
                ...
                <el-button
                  ...
                  size="mini"
                  :disabled="whether_can_revoked(scope.row)"
                  @click="revoke_kgistSelection(scope.row)">撤销</el-button>
              </template>
            </el-table-column>
          </el-table>
        </keep-alive>
        ...
        <el-dialog title="异常修改" :visible.sync="dialogRelationErrorFormVisible" customClass="customWidth" fullscreen>
          ...
          <div slot="footer" class="dialog-footer">
            <el-button @click="dialogRelationErrorFormVisible = false">取 消</el-button>
            <el-button type="primary" @click="dialogRelationErrorFormVisible = false;submit_an_kgist_relation_error_modification();">确 定</el-button>
          </div>
        </el-dialog>
        <!-- <el-button type="primary" @click="get_attribute_error_list()">开始检测</el-button>
        <el-popover placement="top" v-model="isJoin"></el-popover>
          <p>是否应用属性值异常的修改结果？</p>
          <div style="text-align: right; margin: 0">
            <el-button size="mini" type="text" @click="isJoin = false">取消</el-button>
        ...
        <keep-alive>
          <el-table
            ...
            <el-table-column label="实体" width="400px">
              <template slot-scope="scope">
        ...
        <el-dialog title="异常修改" :visible.sync="dialogAttributeErrorFormVisible" fullscreen>
          ...
          <el-card shadow="never" style="">
            <el-table
              ...
              <el-table-column prop="attribute_val" label="属性值"></el-table-column>
            </el-table>
      ...
      <div v-if="this.active==2" style="margin-top: 10px;" v-loading = submitLoading element-loading-text="正在提交中...">
        <el-table :data="tableData4" border style="width: 100%; margin-top: 10px;">
          ...
          <el-table-column label="关系（新）">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ scope.row.rel_new }}</span>
            </template>
          </el-table-column>
          <el-table-column label="尾实体（新）">
            <template slot-scope="scope">
  ...
  export default {
    ...
    data() {
      return {
        ...
        data_to_be_submitted_rel_stack_kgist:[],
        ...
        form_of_attribute_error: {
          ent: '', ent_typ: '', attribute:'', attribute_val:'',
          attr_radio: 3,
        },
      }
    },
    methods: {
      ...
      //对单条链接异常做修改(guize)
      submit_an_kgist_relation_error_modification(){
        var row_of_relation_error_modification = {
          head: '', head_typ: '', rel:'', tail:'', tail_typ:'',
          head_new: '-', head_typ_new: '-', rel_new: '-', tail_new: '-', tail_typ_new: '-',
          rel_radio: 0,
        ...
        this.data_to_be_submitted_rel_stack_kgist.push(row_of_relation_error_modification)
      },
      //将链接修改结果加入提交表
      ...
      //将kgist的链接修改结果加入提交表
      join_kgistSubmission_form(){
        for(var i=0;i <this.data_to_be_submitted_rel_stack_kgist.length; i++){
          this.data_to_be_submitted.push(this.data_to_be_submitted_rel_stack_kgist[i])
        }
        //关系错误置零
        this.data_to_be_submitted_rel_stack_kgist = [];
      },
      //接受一条属性异常的dialog
      ...
      //分页表tableData1，数据来自abnormal_from_kgist
      getTableData(){
        //allData为全部数据
      ...
      //分页表tableData2，数据来自relation_error
      getTableData2(){
        //allData为全部数据
      ...
      //分页表tableData3，数据来自attribute_error
      getTableData3(){
        //allData为全部数据
      ...
      //分页表tableData4，数据来自data_to_be_submitted
      getTableData4(){
        //allData为全部数据
      ...
      revoke_kgistSelection(row){
          row.error_status = 0;
          for(var i=0;i<this.data_to_be_submitted_rel_stack_kgist.length;i++){
            if(this.data_to_be_submitted_rel_stack_kgist[i].pointer == row){
              console.log('来这里了')
              this.data_to_be_submitted_rel_stack_kgist.splice(i, 1);
            }
          }
      },
      //待提交->待处理（属性）
