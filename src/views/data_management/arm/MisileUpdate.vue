...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.put('http://localhost:8181/misile/update',this.ruleForm).then(function(resp){
  ...
  created() {
    const _this = this
    axios.get('http://localhost:8181/misile/findById/'+this.$route.query.id).then(function(resp){
