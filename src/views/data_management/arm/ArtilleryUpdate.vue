...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.put('http://localhost:8181/artillery/update',this.ruleForm).then(function(resp){
  ...
  created() {
    const _this = this
    axios.get('http://localhost:8181/artillery/findById/'+this.$route.query.id).then(function(resp){
