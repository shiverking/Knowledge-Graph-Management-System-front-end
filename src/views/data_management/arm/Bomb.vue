      ...
      <el-form ref="formInline" :inline="true" :model="ruleForm" class="demo-form-inline">
        ...
        <el-form-item label="字段名称：">
          <el-select v-model="ruleForm.key" placeholder="请选择字段名称" >
            <el-option label="爆炸物名称" value ="name"></el-option>
...
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      _this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://localhost:8181/bomb/search',{params:_this.ruleForm}).then(function(resp){
    ...
    deleteBook(row){
      const _this = this
      axios.delete('http://localhost:8181/bomb/deleteById/'+row.id).then(function(resp){
