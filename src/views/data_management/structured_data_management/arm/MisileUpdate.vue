...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          _this.axios.put('/api/misile/update',this.ruleForm).then(function(resp){
            if(resp.data == 'success'){
              _this.$alert('《'+_this.ruleForm.name+'》修改成功！', '消息', {
                confirmButtonText: '确定',
                callback: action => {
                  _this.$router.push('/data/structure/arm/misile')
                }
              })
            }
          })
  ...
  created() {
    const _this = this
    _this.axios.get('/api/misile/findById/'+this.$route.query.id).then(function(resp){
